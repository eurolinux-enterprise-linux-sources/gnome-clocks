/* window.c generated by valac 0.26.0.13-370c, the Vala compiler
...
static void __lambda44_ (ClocksWindow* self);
...
static void __lambda45_ (ClocksWindow* self);
static void ___lambda45__g_object_notify (GObject* _sender, GParamSpec* pspec, gpointer self);
static void __lambda46_ (Block1Data* _data1_);
static void ___lambda46__gtk_widget_destroy (GtkWidget* _sender, gpointer self);
static void __lambda47_ (Block1Data* _data1_, ClocksAlarmFace* w);
void clocks_world_face_reset_view (ClocksWorldFace* self);
static void ___lambda47__clocks_alarm_face_ring (ClocksAlarmFace* _sender, gpointer self);
static void __lambda48_ (Block1Data* _data1_, GParamSpec* w);
...
static void ___lambda48__g_object_notify (GObject* _sender, GParamSpec* pspec, gpointer self);
static void __lambda49_ (Block1Data* _data1_, ClocksTimerFace* w);
static void ___lambda49__clocks_timer_face_ring (ClocksTimerFace* _sender, gpointer self);
static void __lambda50_ (Block1Data* _data1_, GParamSpec* w);
...
static void block1_data_unref (void * _userdata_) {
	...
	if (g_atomic_int_dec_and_test (&_data1_->_ref_count_)) {
		ClocksWindow* self;
		self = _data1_->self;
		_g_object_unref0 (_data1_->timer);
		_g_object_unref0 (_data1_->stopwatch);
		_g_object_unref0 (_data1_->world);
...
static void __lambda48_ (Block1Data* _data1_, GParamSpec* w) {
	...
	_tmp1_ = _data1_->stopwatch;
	_tmp2_ = _data1_->stopwatch;
...
static void __lambda50_ (Block1Data* _data1_, GParamSpec* w) {
	...
	g_return_if_fail (w != NULL);
	_tmp0_ = self->priv->stack;
	_tmp1_ = _data1_->timer;
	_tmp2_ = _data1_->timer;
...
ClocksWindow* clocks_window_construct (GType object_type, ClocksApplication* app) {
	...
	_tmp51_ = g_signal_connect_object ((GObject*) _tmp50_, "notify::visible-child", (GCallback) ___lambda44__g_object_notify, self, 0);
	_data1_->stack_id = _tmp51_;
	_tmp52_ = self->priv->header_bar;
	_tmp53_ = g_signal_connect_object ((GObject*) _tmp52_, "notify::mode", (GCallback) ___lambda45__g_object_notify, self, 0);
	_data1_->header_bar_id = _tmp53_;
	_tmp54_ = self->priv->stack;
	g_signal_connect_data ((GtkWidget*) _tmp54_, "destroy", (GCallback) ___lambda46__gtk_widget_destroy, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
	_tmp55_ = alarm;
	g_signal_connect_data (_tmp55_, "ring", (GCallback) ___lambda47__clocks_alarm_face_ring, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
	_tmp56_ = _data1_->stopwatch;
	g_signal_connect_data ((GObject*) _tmp56_, "notify::state", (GCallback) ___lambda48__g_object_notify, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
	_tmp57_ = _data1_->timer;
	g_signal_connect_data (_tmp57_, "ring", (GCallback) ___lambda49__clocks_timer_face_ring, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
	_tmp58_ = _data1_->timer;
	g_signal_connect_data ((GObject*) _tmp58_, "notify::state", (GCallback) ___lambda50__g_object_notify, block1_data_ref (_data1_), (GClosureNotify) block1_data_unref, 0);
	_tmp59_ = self->priv->stack;
	_tmp60_ = self->priv->panels;
	_tmp60__length1 = self->priv->panels_length1;
	_tmp61_ = self->priv->settings;
	_tmp62_ = g_settings_get_enum (_tmp61_, "panel-id");
	_tmp63_ = _tmp60_[_tmp62_];
	gtk_stack_set_visible_child (_tmp59_, _tmp63_);
	clocks_window_update_header_bar (self);
...
static gboolean clocks_window_real_key_press_event (GtkWidget* base, GdkEventKey* event) {
	...
	if (_tmp0_) {
		...
		_tmp6_ = _tmp5_;
		_tmp7_ = clocks_clock_escape_pressed (G_TYPE_CHECK_INSTANCE_CAST (_tmp6_, CLOCKS_TYPE_CLOCK, ClocksClock));
		result = _tmp7_;
		return result;
	}
	_tmp8_ = event;
	_tmp9_ = GTK_WIDGET_CLASS (clocks_window_parent_class)->key_press_event ((GtkWidget*) G_TYPE_CHECK_INSTANCE_CAST (self, gtk_application_window_get_type (), GtkApplicationWindow), _tmp8_);
	result = _tmp9_;
	return result;
}
