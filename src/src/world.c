/* world.c generated by valac 0.32.0.61-f103e, the Vala compiler
...
static void ___lambda27_ (ClocksWorldFace* self, GObject* obj, GAsyncResult* res);
static void ____lambda27__gasync_ready_callback (GObject* source_object, GAsyncResult* res, gpointer self);
static void __lambda28_ (ClocksWorldFace* self, guint position, guint removed, guint added);
static void clocks_world_face_save (ClocksWorldFace* self);
static void ___lambda28__g_list_model_items_changed (GListModel* _sender, guint position, guint removed, guint added, gpointer self);
...
static void __lambda30_ (ClocksWorldFace* self, ClocksContentItem* l);
static void ___lambda30__clocks_content_store_foreach_func (ClocksContentItem* item, gpointer self);
...
static void __lambda25_ (Block3Data* _data3_, GWeatherLocation* found_location);
ClocksContentItem* clocks_content_store_find (ClocksContentStore* self, ClocksContentStoreFindFunc func, void* func_target);
static gboolean __lambda26_ (Block3Data* _data3_, ClocksContentItem* l);
gboolean clocks_geo_info_is_location_similar (ClocksGeoInfo* self, GWeatherLocation* location);
static gboolean ___lambda26__clocks_content_store_find_func (ClocksContentItem* item, gpointer self);
void clocks_content_store_add (ClocksContentStore* self, ClocksContentItem* item);
static void ___lambda25__clocks_geo_info_location_changed (ClocksGeoInfo* _sender, GWeatherLocation* location, gpointer self);
...
static void __lambda31_ (ClocksWorldFace* self, GtkDialog* dialog, gint response);
...
static void clocks_world_item_class_init (ClocksWorldItemClass * klass) {
	...
	g_object_class_install_property (G_OBJECT_CLASS (klass), CLOCKS_WORLD_ITEM_SELECTED, g_param_spec_boolean ("selected", "selected", "selected", FALSE, G_PARAM_STATIC_NAME | G_PARAM_STATIC_NICK | G_PARAM_STATIC_BLURB | G_PARAM_READABLE | G_PARAM_WRITABLE));
...
static void _vala_clocks_world_item_get_property (GObject * object, guint property_id, GValue * value, GParamSpec * pspec) {
	...
	switch (property_id) {
		...
		break;
		case CLOCKS_WORLD_ITEM_SELECTED:
...
ClocksWorldLocationDialog* clocks_world_location_dialog_construct (GType object_type, GtkWindow* parent, ClocksWorldFace* world_face) {
	...
	ClocksWorldFace* _tmp1_ = NULL;
	ClocksWorldFace* _tmp2_ = NULL;
	...
	if (g_strcmp0 (_tmp1_, "") != 0) {
		...
		if (_tmp4_) {
			...
			if (_tmp12_ == NULL) {
				...
				_tmp15_ = _tmp14_;
				g_warning ("world.vala:216: Timezone not defined for %s. This is a bug in libgweat" \
...
static void ____lambda27__gasync_ready_callback (GObject* source_object, GAsyncResult* res, gpointer self) {
	___lambda27_ ((ClocksWorldFace*) self, source_object, res);
	g_object_unref (self);
}
...
static void ___lambda28__g_list_model_items_changed (GListModel* _sender, guint position, guint removed, guint added, gpointer self) {
	__lambda28_ ((ClocksWorldFace*) self, position, removed, added);
}
...
static void __lambda30_ (ClocksWorldFace* self, ClocksContentItem* l) {
	...
	_tmp0_ = l;
	clocks_world_item_tick (G_TYPE_CHECK_INSTANCE_CAST (_tmp0_, CLOCKS_WORLD_TYPE_ITEM, ClocksWorldItem));
}


static void ___lambda30__clocks_content_store_foreach_func (ClocksContentItem* item, gpointer self) {
	__lambda30_ ((ClocksWorldFace*) self, item);
}
...
static void __lambda29_ (ClocksWorldFace* self) {
	...
	clocks_content_store_foreach (_tmp0_, ___lambda30__clocks_content_store_foreach_func, self);
...
ClocksWorldFace* clocks_world_face_construct (GType object_type, ClocksHeaderBar* header_bar) {
	...
	ClocksHeaderBar* _tmp21_ = NULL;
	GtkButton* _tmp22_ = NULL;
	ClocksContentView* _tmp23_ = NULL;
	ClocksContentStore* _tmp24_ = NULL;
	ClocksContentView* _tmp25_ = NULL;
	ClocksHeaderBar* _tmp26_ = NULL;
	GSettings* _tmp27_ = NULL;
	gboolean _tmp28_ = FALSE;
	ClocksContentStore* _tmp29_ = NULL;
	ClocksUtilsWallClock* _tmp30_ = NULL;
	...
	clocks_content_store_set_sorting (_tmp4_, ___lambda22__gcompare_data_func, g_object_ref (self), g_object_unref);
	...
	_tmp15_ = (GtkImage*) gtk_image_new_from_icon_name ("go-previous-symbolic", GTK_ICON_SIZE_MENU);
	...
	gtk_button_set_image (_tmp17_, (GtkWidget*) _tmp18_);
	...
	gtk_header_bar_pack_start ((GtkHeaderBar*) _tmp21_, (GtkWidget*) _tmp22_);
	_tmp23_ = self->priv->content_view;
	_tmp24_ = self->priv->locations;
	clocks_content_view_bind_model (_tmp23_, _tmp24_);
	_tmp25_ = self->priv->content_view;
	_tmp26_ = header_bar;
	clocks_content_view_set_header_bar (_tmp25_, _tmp26_);
	clocks_world_face_load (self);
	gtk_widget_show_all ((GtkWidget*) self);
	_tmp27_ = self->priv->settings;
	_tmp28_ = g_settings_get_boolean (_tmp27_, "geolocation");
	if (_tmp28_) {
		clocks_world_face_use_geolocation (self, ____lambda27__gasync_ready_callback, g_object_ref (self));
	}
	_tmp29_ = self->priv->locations;
	g_signal_connect_object ((GListModel*) _tmp29_, "items-changed", (GCallback) ___lambda28__g_list_model_items_changed, self, 0);
	clocks_world_face_reset_view (self);
	_tmp30_ = clocks_utils_wall_clock_get_default ();
	_tmp31_ = _tmp30_;
	g_signal_connect_object (_tmp31_, "tick", (GCallback) ___lambda29__clocks_utils_wall_clock_tick, self, 0);
...
static void block3_data_unref (void * _userdata_) {
	...
	if (g_atomic_int_dec_and_test (&_data3_->_ref_count_)) {
		ClocksWorldFace* self;
		self = _data3_->self;
		_g_object_unref0 (_data3_->geo_info);
...
static gboolean __lambda26_ (Block3Data* _data3_, ClocksContentItem* l) {
	...
	_tmp1_ = clocks_world_item_get_location (G_TYPE_CHECK_INSTANCE_CAST (_tmp0_, CLOCKS_WORLD_TYPE_ITEM, ClocksWorldItem));
	_tmp2_ = _tmp1_;
	_tmp3_ = clocks_geo_info_is_location_similar (_data3_->geo_info, _tmp2_);
	result = _tmp3_;
	return result;
}
...
static void __lambda25_ (Block3Data* _data3_, GWeatherLocation* found_location) {
	...
	ClocksWorldItem* _tmp7_ = NULL;
	ClocksContentStore* _tmp8_ = NULL;
	ClocksWorldItem* _tmp9_ = NULL;
	self = _data3_->self;
	g_return_if_fail (found_location != NULL);
	_tmp0_ = self->priv->locations;
	_tmp1_ = clocks_content_store_find (_tmp0_, ___lambda26__clocks_content_store_find_func, _data3_);
	...
	_tmp3_ = found_location;
	_tmp4_ = clocks_world_item_new (_tmp3_);
	_g_object_unref0 (item);
	item = _tmp4_;
	_tmp5_ = item;
	clocks_world_item_set_automatic (_tmp5_, TRUE);
	_tmp6_ = item;
	clocks_content_item_set_selectable ((ClocksContentItem*) _tmp6_, FALSE);
	_tmp7_ = item;
	clocks_content_item_set_title_icon ((ClocksContentItem*) _tmp7_, "find-location-symbolic");
	_tmp8_ = self->priv->locations;
	_tmp9_ = item;
	clocks_content_store_add (_tmp8_, (ClocksContentItem*) _tmp9_);
	_g_object_unref0 (item);
}


static void ___lambda25__clocks_geo_info_location_changed (ClocksGeoInfo* _sender, GWeatherLocation* location, gpointer self) {
	__lambda25_ (self, location);
}
...
static gboolean clocks_world_face_use_geolocation_co (ClocksWorldFaceUseGeolocationData* _data_) {
	...
	_data_->_data3_->geo_info = _data_->_tmp0_;
	g_signal_connect_data (_data_->_data3_->geo_info, "location-changed", (GCallback) ___lambda25__clocks_geo_info_location_changed, block3_data_ref (_data_->_data3_), (GClosureNotify) block3_data_unref, 0);
	_data_->_state_ = 1;
	clocks_geo_info_seek (_data_->_data3_->geo_info, clocks_world_face_use_geolocation_ready, _data_);
	return FALSE;
	_state_1:
	clocks_geo_info_seek_finish (_data_->_data3_->geo_info, _data_->_res_);
	...
	n = _tmp1_;
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				ClocksContentStore* _tmp6_ = NULL;
				gint _tmp7_ = 0;
				GObject* _tmp8_ = NULL;
				ClocksWorldItem* _tmp9_ = NULL;
...
static void ___lambda31__gtk_dialog_response (GtkDialog* _sender, gint response_id, gpointer self) {
	__lambda31_ ((ClocksWorldFace*) self, _sender, response_id);
}
...
static void clocks_world_face_real_activate_new (ClocksClock* base) {
	...
	dialog = _tmp1_;
	g_signal_connect_object ((GtkDialog*) dialog, "response", (GCallback) ___lambda31__gtk_dialog_response, self, 0);
...
static void clocks_world_face_real_update_header_bar (ClocksClock* base) {
	...
	switch (_tmp3_) {
		...
		case CLOCKS_HEADER_BAR_MODE_STANDALONE:
		{
			...
			gchar* _tmp12_ = NULL;
			ClocksHeaderBar* _tmp13_ = NULL;
			ClocksHeaderBar* _tmp14_ = NULL;
			ClocksWorldItem* _tmp15_ = NULL;
			gchar* _tmp16_ = NULL;
			gchar* _tmp17_ = NULL;
			gchar* _tmp18_ = NULL;
			GtkButton* _tmp19_ = NULL;
			...
			_tmp11_ = _tmp10_;
			_tmp12_ = _tmp11_;
			gtk_header_bar_set_title ((GtkHeaderBar*) _tmp8_, _tmp12_);
			_g_free0 (_tmp12_);
			_tmp13_ = clocks_clock_get_header_bar ((ClocksClock*) self);
			_tmp14_ = _tmp13_;
			_tmp15_ = self->priv->standalone_location;
			_tmp16_ = clocks_world_item_get_contry_name (_tmp15_);
			_tmp17_ = _tmp16_;
			_tmp18_ = _tmp17_;
			gtk_header_bar_set_subtitle ((GtkHeaderBar*) _tmp14_, _tmp18_);
			_g_free0 (_tmp18_);
			_tmp19_ = self->priv->back_button;
