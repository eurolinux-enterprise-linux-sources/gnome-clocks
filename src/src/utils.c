/* utils.c generated by valac 0.26.0.13-370c, the Vala compiler
...
static void __lambda31_ (ClocksUtilsBell* self, ca_context* c, guint32 id, gint code);
static gboolean _clocks_utils_bell_keep_ringing_gsource_func (gpointer self);
static void ___lambda31__ca_finishcallback (ca_context* c, guint32 id, gint code, gpointer self);
...
GdkPixbuf* clocks_utils_load_image (const gchar* image) {
	...
	__catch3_g_error:
	{
		...
		g_warning ("utils.vala:40: loading image file: %s", _tmp6_);
		_g_error_free0 (e);
	}
	__finally3:
	if (G_UNLIKELY (_inner_error_ != NULL)) {
...
static ClocksUtilsWallClock* clocks_utils_wall_clock_construct (GType object_type) {
	...
	_tmp6_ = self->priv->wc;
	g_signal_connect_object ((GObject*) _tmp6_, "notify::timezone", (GCallback) ___lambda7__g_object_notify, self, 0);
	_tmp7_ = g_settings_new ("org.gnome.desktop.interface");
	_g_object_unref0 (self->priv->settings);
	self->priv->settings = _tmp7_;
	_tmp8_ = self->priv->settings;
	g_signal_connect_object (_tmp8_, "changed::clock-format", (GCallback) ___lambda8__g_settings_changed, self, 0);
	clocks_utils_wall_clock_update_format (self);
	clocks_utils_wall_clock_update (self);
	return self;
}
	...
	d = _tmp0_;
	{
		...
		while (TRUE) {
			...
			if (_tmp8_) {
				...
				gint32 _tmp13_ = 0;
				_tmp13_ = i;
				g_warning ("utils.vala:279: Invalid days %d", (gint) _tmp13_);
			}
		}
		_g_variant_unref0 (v);
...
ClocksUtilsBell* clocks_utils_bell_construct (GType object_type, const gchar* soundid) {
	...
	if (_tmp2_ < 0) {
		g_warning ("utils.vala:296: Sound will not be available");
		_ca_context_destroy0 (self->priv->canberra);
...
static gboolean _clocks_utils_bell_keep_ringing_gsource_func (gpointer self) {
	gboolean result;
	result = clocks_utils_bell_keep_ringing ((ClocksUtilsBell*) self);
	return result;
}
...
static void ___lambda31__ca_finishcallback (ca_context* c, guint32 id, gint code, gpointer self) {
	__lambda31_ ((ClocksUtilsBell*) self, c, id, code);
}
...
static gboolean clocks_utils_bell_keep_ringing (ClocksUtilsBell* self) {
	...
	ca_context_play_full (_tmp3_, (guint32) 1, pl, ___lambda31__ca_finishcallback, self);
	result = FALSE;
	_ca_proplist_destroy0 (pl);
	return result;
}
